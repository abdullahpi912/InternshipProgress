...
The backend was tested independently with Postman before trusting the
frontend — happy path, invalid inputs, duplicate email, and invalid
endpoint/method requests. See `docs/day-45-api-testing.md` for the full
report and `postman/Student_Management_API.postman_collection.json` for the
collection (import it into Postman, set the `base_url` variable, and run
against a locally running server).

Testing surfaced one gap: Flask's default 404/405 handlers return HTML, not
JSON, which an API client can't use. Fixed with JSON `errorhandler`s for
404/405/500 in `app.py`, so every response — success or error — is now
